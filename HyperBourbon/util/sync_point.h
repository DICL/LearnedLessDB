...
#define TEST_SYNC_POINT(x) leveldb::SyncPoint::GetInstance()->Process(x)
#define TEST_IDX_SYNC_POINT(x, index) \
  leveldb::SyncPoint::GetInstance()->Process(x + std::to_string(index))
#define TEST_SYNC_POINT_CALLBACK(x, y) \
  leveldb::SyncPoint::GetInstance()->Process(x, y)
#define INIT_SYNC_POINT_SINGLETONS() \
  (void)leveldb::SyncPoint::GetInstance();
#endif  // NDEBUG
#endif		// MIXGRAPH
