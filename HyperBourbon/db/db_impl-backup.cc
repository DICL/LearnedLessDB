...
DBImpl::DBImpl(const Options& raw_options, const std::string& dbname)
  ...
  num_bg_threads_ = 9;
#if OFFLINE_LEARNING
	koo::num_bg_threads_off.store(5);
#endif
#else
  num_bg_threads_ = 2;
#endif
...
DBImpl::~DBImpl() {
...
#if OFFLINE_LEARNING
	off_learning_cv_.SignalAll();		// TODO 필요?
#endif
  ...
  if (db_lock_ != NULL) {
	...
	if (koo::num_files_flush) {
		...
		std::cout << "\n# input files of compaction triggered after load = " << koo::num_inputs_compaction_triggered_after_load << std::endl;
		std::cout << "# output files of compaction triggered after load = " << koo::num_outputs_compaction_triggered_after_load << std::endl;
	...
	if (koo::num_learntime) {
		std::cout << "----------------------------------------------------------" << std::endl;
		std::cout << "Avg compaction time = " << std::to_string((double)koo::compactiontime/(double)koo::num_compactiontime) << " ns,\t# compaction = " << koo::num_compactiontime << ",\tTotal compaction time = " << std::to_string(koo::compactiontime) << std::endl;
#if AC_TEST
	...
	if (!koo::lifespans.empty()) {
		...
		std::ofstream ofs("/koo/HyperBourbon/koo/data/lifespans.txt");*/
		...
		for (int i=0; i<config::kNumLevels; i++) {
			...
			std::cout << "\t\tAvg. M_time: " << std::to_string(total_M_time_pos[i]/(double)num_M_time_pos[i]) << ",\t\tTotal: " << total_M_time_pos[i] << ",\tnum: " << num_M_time_pos[i] << std::endl;
...
/*#if MULTI_COMPACTION				// TODO MaybeAddFile() assertion 난다
		while (!shutting_down_.Acquire_Load()) {
			if (imm_ != NULL) break;
			mutex_.Unlock();
      env_->SleepForMicroseconds(100000);			// 0.1s
			mutex_.Lock();
		}
#else*/
#if OFFLINE_LEARNING
    while ((!shutting_down_.Acquire_Load() && imm_ == NULL) || koo::wait_for_learning) {

    	koo::num_bg_threads_off -= 1;
			if (koo::wait_for_learning) off_learning_cv_.Signal();
#else
...
#endif
#if OFFLINE_LEARNING
    	if (level != config::kNumLevels && !koo::wait_for_learning) break;
    	koo::num_bg_threads_off -= 1;
			if (koo::wait_for_learning) off_learning_cv_.Signal();
#else
    	if (level != config::kNumLevels) break;
...
#if MULTI_COMPACTION_DEBUG
			std::thread::id t_id = std::this_thread::get_id();
			std::cout << "\n<Thread " << t_id << "> Pick Compaction (level: " << level << " -> " << level+1 << ")\n";
#endif
...
#if MULTI_COMPACTION_DEBUG1
		std::thread::id t_id = std::this_thread::get_id();
		std::cout << "<Thread " << t_id << "> (Trivial Move) Compaction level: " << c->level() << " -> " << c->level()+1 << " ... \n";
		std::cout << "\t# inputs = " << c->num_input_files(0) << std::endl;
#endif
...
/*#if LEARN
...
#if MULTI_COMPACTION_DEBUG
		std::cout << "\t# outputs = " << c->num_input_files(0) << std::endl;
		std::cout << "\tfinished Compaction <Thread " << t_id << ">\n";
#endif
#if MULTI_COMPACTION_CNT
...
#if MULTI_COMPACTION_DEBUG1
		std::thread::id t_id = std::this_thread::get_id();
		std::cout << "<Thread " << t_id << "> Compaction level: " << c->level() << " -> " << c->level()+1 << " ... \n";
		std::cout << "\t# inputs = " << c->num_input_files(0) << " + " << c->num_input_files(1) << std::endl;
#endif
    ...
    status = DoCompactionWork(compact);
		...
		if (koo::count_compaction_triggered_after_load) {
			...
			koo::num_inputs_compaction_triggered_after_load += c->num_input_files(0) + c->num_input_files(1);
#if TIME_W_DETAIL
...
#if MULTI_COMPACTION_DEBUG
	std::thread::id t_id = std::this_thread::get_id();
	std::cout << "\n\t<Thread " << t_id << "> Compaction Outputs (level " << compact->compaction->level()+1 << "):\n";
	std::cout << "\tnum: " << compact->outputs.size() << std::endl;
	for (auto& output : compact->outputs) {
		std::cout << "\t" << output.number << ": " << output.smallest.user_key().ToString() << " ~ " << output.largest.user_key().ToString() << "(size: " << output.file_size << ")" << std::endl;
	}
#endif
...
/*#if LEARN
		...
		koo::Stats* instance = koo::Stats::GetInstance();
		for (auto& output : compact->outputs) {
			uint32_t dummy;			// TODO dummy를 output 안에 각각 저장해야 할수도
			FileMetaData* meta = new FileMetaData();
			meta->number = output.number;
			meta->file_size = output.file_size;
			meta->smallest = output.smallest;
...
#endif
#if REMOVE_MUTEX
  mem->UnrefForGet(&mutex_);
  if (imm != NULL) imm->UnrefForGet(&mutex_);
  current->UnrefForGet(&mutex_);
#else
  mem->Unref();
...
void DBImpl::WaitForBackground() {
	mutex_.Lock();
	koo::wait_for_learning = true;
	//shutting_down_.Release_Store(this);
	mutex_.Unlock();
	//bg_compaction_cv_.SignalAll();
	//bg_memtable_cv_.SignalAll();
  while (koo::num_bg_threads_off > 0) {
  	std::cout << "WaitForBackground() " << koo::num_bg_threads_off << std::endl;
    off_learning_cv_.Wait();
  }
}

void DBImpl::FileLearn() {
	std::cout << "WaitForBackground() start\n";
	WaitForBackground();
	std::cout << "WaitForBackground() end\n";
  //mutex_.AssertHeld();
	//mutex_.Lock();
	Version* ver = versions_->current();
	//ver->Ref();
	//mutex_.Unlock();
	std::cout << "FileLearn() start\n";
	ver->FileLearn();
	std::cout << "FileLearn() end\n";
	//mutex_.Lock();
	//ver->Unref();
	koo::wait_for_learning = false;
  bg_fg_cv_.SignalAll();
	mutex_.Unlock();
	//bg_compaction_cv_.Signal();
	//bg_memtable_cv_.Signal();

	/*shutting_down_.Release_Store(NULL);
	bg_compaction_cv_.SignalAll();
	bg_memtable_cv_.SignalAll();*/
}
#endif
