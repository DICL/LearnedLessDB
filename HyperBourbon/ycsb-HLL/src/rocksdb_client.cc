...
#include <atomic>
#include "/koo/HyperBourbon/util/random.h"
#include "koo/koo.h"
...
RocksDBClient::RocksDBClient(WorkloadProxy* workload_proxy, int num_threads,
    ...
    //read_finished(0) {
#if YCSB_LOAD_THREAD
	if (num_threads_ < YCSB_LOAD_THREAD) {
...
#if YCSB_LOAD_THREAD
	uint64_t num = load_num_ / YCSB_LOAD_THREAD;
#else
...
#if AC_TEST
	printf("------------------------------------------------------------------\n");
	printf("# generated files by flush: %lu\n# generated files by compaction: %lu\n# learned files: %lu\n# merged files: %lu\n", koo::num_files_flush, koo::num_files_compaction, koo::num_learned, koo::num_merged);
	printf("# compaction triggered after load: %lu\n", koo::num_compaction_triggered_after_load);
	printf("------------------------------------------------------------------\n");
#endif

  // Record results to a file

#if !YCSB_WRAPPER && !YCSB_SKIP_LOAD
...
#if LEARNING_IO
	std::cout << "Client thread id: " << syscall(SYS_gettid) << std::endl;
#endif
...
#if YCSB_LOAD_THREAD
		int num_threads__ = workload_proxy_->is_load() ? YCSB_LOAD_THREAD : num_threads_;
