...
void Table::TestModelAccuracy(uint64_t& file_number) {
	...
	while (iiter->Valid()) {
		...
		if (handle.DecodeFrom(&handle_value).ok()) {
			...
			while (block_iter->Valid()) {
				ParseInternalKey(block_iter->key(), &parsed_key);
				keys.emplace_back(parsed_key.user_key.SliceToInteger());
				//keys.emplace_back(parsed_key.user_key.key_num());
				//std::string string_key(parsed_key.user_key.data(), parsed_key.user_key.size());
				//keys.emplace_back(atoll(string_key.c_str()));
				//keys.emplace_back(std::stoull(string_key));
				block_iter->Next();
			}
	...
	std::vector<koo::Segment> segs;
	if (model->is_retrained_) segs = model->string_segments_bak;
	else segs = model->string_segments;
	std::vector<uint64_t> *keys = model->string_keys;
	...
	for (int i=0; i<segs_size-1; i++) {			// dummy segment
		...
		for (uint64_t j=start; j<keys_size; j++) {
...
#if NORMARLIZE_KEY
			double x_real = static_cast<double>(key - model->min_key + 1);
#else
			...
			str_error += std::to_string(diff) + "\n";
			str_diff += "(" + std::to_string(key) + ", " + std::to_string(j) + ")\t" + std::to_string(y_inf) + "\t\tDiff = " + std::to_string(diff) + "\t\tx_double: " + std::to_string(x_real) + "\n";
			diff_sum += diff;
			if (diff > diff_max) {
				diff_max = diff;
				max_idx = j;
	...
	if (!skip) {
		if (model->Merged()) {
			koo::mm_num_error2[model->level]++;
			koo::mm_avg_error[model->level] += diff_avg_uint;
			koo::mm_max_error[model->level] += diff_max_uint;
			if (koo::mm_avg_error[model->level] > 1000000) {
				uint64_t tmp = koo::mm_avg_error[model->level].load();
				fprintf(stderr, "ERRRROOOOOORRRRRRRRRRR21 fn: %lu, %lu\n", model->file_number, tmp); 
				write = true;
			}
			if (diff_max > error_bound) {
				koo::mm_num_error_over[model->level]++;
				koo::mm_avg_error_over[model->level] += diff_avg_uint;
				koo::mm_max_error_over[model->level] += diff_max_uint;
				koo::mm_cnt_overmax[model->level] += static_cast<uint64_t>(cnt_overmax);
			}
		} else {
			koo::lm_num_error2[model->level]++;
		...
		of_stdev.open("/koo/HyperLearningless3/koo/data/error_avg_stdev_level"+std::to_string(model->level)+".txt", std::ios::app);
		of_stdev << diff_avg << "\n";
		of_stdev.close();
		of_stdev.open("/koo/HyperLearningless3/koo/data/error_max_stdev_level"+std::to_string(model->level)+".txt", std::ios::app);
		of_stdev << diff_max << "\n";
		of_stdev.close();*/
		/*of_stdev.open("/koo/HyperLearningless3/koo/data/model_error_"+std::to_string(file_number)+".txt");
		of_stdev << str_error << "\n";
		of_stdev.clos();*/
	}
