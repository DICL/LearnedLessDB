...
int main(int argc, char *argv[]) {
    ...
    string db_location, input_filename, distribution_filename, ycsb_filename;
    ...
    commandline_options.add_options()
            ...
            ("YCSB", "use YCSB trace", cxxopts::value<string>(ycsb_filename)->default_value(nullptr))
    ...
    uint64_t write_i = 0;
        ...
        if (write) {
					if (input_filename.empty()) {			// used for ycsb default
						//status = db->Put(write_options, generate_key(to_string(distribution[i])), {values.data() + uniform_dist_value(e3), (uint64_t)value_size});
            ...
            if (use_ycsb && ycsb_is_write[i] == 2) {		// ycsb insert
              //status = db->Put(write_options, generate_key(to_string(10000000000 + index)), {values.data() + uniform_dist_value(e3), (uint64_t) value_size});
              status = db->Put(write_options, generate_key(to_string(10000000000 + index)), w_value);
            } else {		// other write
              //status = db->Put(write_options, keys[index], {values.data() + uniform_dist_value(e3), (uint64_t)value_size});
