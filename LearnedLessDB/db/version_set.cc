...
bool Version::GetOverlappingInputsNotBeingCompacted(
  ...
  const Comparator* user_cmp = vset_->icmp_.user_comparator();
  size_t files_size = files_[level].size();
  for (size_t i = 0; i < files_size; ) {
    FileMetaData* f = files_[level][i++];
    const Slice file_start = f->smallest.user_key();
    const Slice file_limit = f->largest.user_key();
    if (begin != NULL && user_cmp->Compare(file_limit, user_begin) < 0) {
...
#if MULTI_COMPACTION
			int num_files = v->files_[0].size() - num_being_compacted[0];
  ...
  if (level > 0) {
...
#if MULTI_COMPACTION
				if (LA[j]->being_compacted) break;
				bool nothing_being_compacted = true;
				for (size_t k=boundaries[j].start; k<boundaries[j].limit; ++k) {
    ...
    // otherwise just pick the file with least overlap
      ...
      for (size_t i=0; i<boundaries_size; ++i) {
        ...
        if (set_smallest) {
          bool nothing_being_compacted = true;
          for (size_t j=boundaries[i].start; j<boundaries[i].limit; ++j) {
            if (LB[j]->being_compacted) {
              nothing_being_compacted = false;
              break;
            }
          }
          if (!nothing_being_compacted) continue;
          set_smallest = false;
          smallest = i;
          continue;
        }
        if (boundaries[smallest].limit - boundaries[smallest].start >
...
void VersionSet::RegisterCompaction(Compaction* c) {
	...
	} else compactions_in_progress_[c->level()].insert(c);
	for (size_t which=0; which<2; which++)
		CountNumBeingCompacted(c->level()+which, true, c->inputs_[which].size());
}

void VersionSet::UnregisterCompaction(Compaction* c) {
	for (size_t which=0; which<2; which++)
...
void Compaction::MarkFilesBeingCompacted(bool mark_as_compacted) {
	for (size_t which=0; which<2; which++) {
		for (auto* f : inputs_[which]) {
			assert(mark_as_compacted ? !f->being_compacted
															 : f->being_compacted);
			f->being_compacted = mark_as_compacted;
		}
	}
}
#endif
...
/*#if RETRAIN2 && AC_TEST
			...
			auto* m = koo::file_data->GetModel(meta->number);
			ofs << meta->number << ": " << m->extended_cnt << ", " << m->actual_extended_cnt << std::endl;
			ofs.close();
#endif*/
#if MODEL_ACCURACY
				if (model->Merged()) {
